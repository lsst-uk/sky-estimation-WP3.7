...
TO FIX: ut is local right now.  Include relevant functions here!
"""
...
def makeSkyMapBinning(skyImage, binnedHdu, sigma=1.0, block=16):
    ...
    wid = 4
    padBin = np.pad(binnedHdu[0].data, wid, constant_values=np.nan)
    ...
    # Smooth the image
    binned = ndimage.gaussian_filter(binned, sigma, mode='reflect')
    binned = binned[wid:-wid, wid:-wid]
...
def makeSkyMapWavelet(skyImage, method='BayesShrink', mode='soft',
                      wavelet='db1'):
    '''
    Temporary code, for exploration purposes.  Tries wavelet denoising.
    Slightly slower than above algorithm, and produces similar results.

     Gaussian mixture modeling?
    '''
    skyMap = denoise_wavelet(skyImage[0].data, channel_axis=None,
                             method=method, mode=mode, wavelet=wavelet,
                             rescale_sigma=True)
...
def finalProcessing(imListFull, imListModels, coaddHdu,
    ...
    for i in range(len(imListFull)):
        ...
        # Kind of. Not around bright stars.
